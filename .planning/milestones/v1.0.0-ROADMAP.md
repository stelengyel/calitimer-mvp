...
Plans:
- [x] 05-01-PLAN.md — HandstandClassifier (pure static) + HoldStateMachine (@MainActor, debounce, backdated timestamps, target alert) + DetectionIndicatorPreference
- [x] 05-02-PLAN.md — HoldIndicatorView + HoldTimerView + wire into LiveSessionView + SessionConfigSheet indicator toggle + SettingsView indicator toggle
- [x] 05-03-PLAN.md — AVAssetReaderScanner (full-speed upload scan) + VideoImportManager scan hook + UploadModeView Zone 3 results list
- [x] 05-04-PLAN.md — Build on device + confirm Vision joint key strings + human verify all 7 Phase 5 requirements
